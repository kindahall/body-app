  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "Tu es un coach de vie et psychologue expert qui analyse les données personnelles pour fournir des insights constructifs et bienveillants. Réponds toujours en français et sois empathique. Utilise un format markdown avec des sections claires. Tu adaptes tes recommandations selon l'âge et la situation de vie de la personne."
        },
        {
          role: "user",
          content: prompt
...
function buildAnalysisPrompt(data: AnalysisData): string {
  ...
  if (userAge) {
    ...
    if (userAge >= 18 && userAge <= 25) {
      ...
      ageContext = 'Période de découverte, d\'exploration et de construction d\'identité. Focus sur l\'apprentissage, l\'indépendance et les premières expériences relationnelles importantes.'
    } else if (userAge >= 26 && userAge <= 35) {
      ageGroup = 'Adulte établi (26-35 ans)'
      ageContext = 'Période de stabilisation et de construction. Focus sur la carrière, les relations sérieuses, possibles projets familiaux et l\'établissement de sa place dans la société.'
    } else if (userAge >= 36 && userAge <= 45) {
      ageGroup = 'Adulte mature (36-45 ans)'
      ageContext = 'Période d\'équilibre et de réalisation. Focus sur l\'accomplissement professionnel, la famille établie, et la recherche d\'épanouissement personnel plus profond.'
    } else if (userAge >= 46 && userAge <= 55) {
      ageGroup = 'Adulte expérimenté (46-55 ans)'
      ageContext = 'Période de sagesse et de transmission. Focus sur le mentorat, la consolidation des acquis, et la préparation de nouvelles étapes de vie.'
    } else if (userAge >= 56 && userAge <= 65) {
      ageGroup = 'Pré-senior (56-65 ans)'
      ageContext = 'Période de transition et de bilan. Focus sur la préparation de la retraite, le bilan de vie, et la recherche de sens et de legacy.'
    } else if (userAge >= 66) {
      ageGroup = 'Senior (66+ ans)'
      ageContext = 'Période de sérénité et de partage. Focus sur la sagesse acquise, la transmission aux générations suivantes, et l\'épanouissement dans une nouvelle phase de vie.'
    }
...
Identifie les patterns principaux dans les relations et comportements${userAge ? `, en tenant compte de l'âge (${userAge} ans)` : ''}

## 💪 Forces et Qualités  
Souligne les points positifs et forces de caractère

## 🎯 Axes d'Amélioration
Suggestions constructives pour le développement personnel${userAge ? ` adaptées à la tranche d'âge` : ''}

## 📋 Recommandations Pratiques
3-4 actions concrètes à mettre en place${userAge ? `, calibrées selon l'âge et le contexte de vie` : ''}
...
${userAge ? `**IMPORTANT**: Calibre tes recommandations selon l'âge de la personne (${userAge} ans). Adapte tes conseils aux défis, opportunités et priorités typiques de cette tranche d'âge. Les attentes et contextes de vie varient significativement selon l'âge. Utilise des références et exemples appropriés à cette génération.` : ''}`
